"""Custom exception hierarchy for BrickSQL.

All public errors inherit from BrickSQLError so callers can catch the base
class for any BrickSQL-specific failure.
"""
...
class BrickSQLError(Exception):
    """Base exception for all BrickSQL errors."""


class ParseError(BrickSQLError):
...
class ValidationError(BrickSQLError):
...
class ProfileConfigError(BrickSQLError):
...
class CompilationError(BrickSQLError):
