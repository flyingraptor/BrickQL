...
> **Scope:** brickQL is a read-only query layer. It supports `SELECT` queries only. `INSERT`, `UPDATE`, `DELETE`, `CREATE`, and all other write or DDL statements are outside its scope by design.
